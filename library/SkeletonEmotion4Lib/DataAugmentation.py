...
class DataAugmentationEncDecGenerator(tf.keras.utils.Sequence):
    def __init__(self, data, batch_size=32, augment_fn=None, shuffle=False):
        '''
...
class DataAugmentationClsGenerator(tf.keras.utils.Sequence):
    def __init__(self, data, labels, batch_size=32, augment_fn=None, shuffle=False):
        '''
        data: Matrix of type Numpy array
        labels: Vector with the labels
        batch_size: Batch size
        augment_fn: Augmented function, receive a numpy array and return a numpy array of same shape.
        shuffle: if shuffle on epoch end
        '''
        self.data = data
        self.labels = labels
        self.batch_size = batch_size
        ...
        self.classes = np.argmax(self.one_hot_labels, axis=1);
